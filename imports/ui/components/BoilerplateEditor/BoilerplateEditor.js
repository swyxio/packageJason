...
class BoilerplateEditor extends React.Component {
  ...
  getDetails() {
    ...
    axios.get(`https://api.github.com/repos/${owner}/${repo}/contents/package.json?client_id=6d9110c2756af4885f35&client_secret=f6c615cb93489d50141b33df68e581b24029db03`).then((response) => {
      this.setState({
        data: response.data,
      });
      this.getTree();
  ...
  getTree() {
    ...
    axios.get(`https://api.github.com/repos/${owner}/${repo}/git/trees/master?recursive=1&client_id=6d9110c2756af4885f35&client_secret=f6c615cb93489d50141b33df68e581b24029db03`).then((response) => {
      // console.log('axios wins', response);
      this.setState({
        tree: response.data,
      });
    }).catch((error) => {
  ...
  getPopularityInfo() {
    ...
    Promise.all([repocall, commitcall, contribcall]).then(([repovar, commitvar, contribvar]) => {
      // console.log('repovar', repovar.data);
      // console.log('commitvar', commitvar.data);
      // console.log('contribvar', contribvar.headers);
      // console.log('contribvar', !contribvar.headers.link ? 1 : contribvar.headers.link.split('&page=')[2].split('>;')[0]);
      // console.log('downloads', commitvar.data.assets[0].download_count);
      // console.log('stargazers_count', repovar.data.stargazers_count);
      // console.log('commitvar', commitvar.data[0].commit.author.date);
      this.setState({
